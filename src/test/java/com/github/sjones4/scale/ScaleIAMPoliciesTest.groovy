...
/**
 * Scale test for IAM user attached policies.
 *
 * https://eucalyptus.atlassian.net/browse/EUCA-11092
 */
class ScaleIAMPoliciesTest {

  private final AWSCredentialsProvider eucalyptusCredentials

  ScaleIAMPoliciesTest( ) {
  ...
  void test( ) {
    ...
    try {
      ...
      final int policies = 2
      final int iterations = accounts / threads
      print( "Creating ${users} users with ${policies} policies in ${accounts} accounts using ${threads} threads" )
          ...
          try{
            ( 1..iterations ).each { Integer account ->
              ...
              getYouAreClient( accountCredentials ).with {
                  ...
                  listUsers( ).with { ListUsersResult result ->
                    result.users.each { User user ->
                      if ( user.userName != 'admin' ) {
                        listUserPolicies(new ListUserPoliciesRequest(userName: user.userName)).with {
                          policyNames.each { String policyName ->
                ...
                print( "[${thread}] Creating ${users} users with ${policies} policies for account ${accountName}" )
                (1..users).each { Integer user ->
                  ...
                  (1..policies).each { Integer policy ->
                    putUserPolicy( new PutUserPolicyRequest( 
                        userName: userName,
                        policyName: "${namePrefix}policy-${thread}-${user}-${policy}",
                        policyDocument: """\
                        {
                           "Statement":[{
                              "Effect":"Allow",

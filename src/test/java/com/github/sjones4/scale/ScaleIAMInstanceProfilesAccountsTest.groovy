...
/**
 * Scale test for IAM instance profiles in multiple accounts.
 *
 * https://eucalyptus.atlassian.net/browse/EUCA-11089
 */
class ScaleIAMInstanceProfilesAccountsTest {

  private final AWSCredentialsProvider eucalyptusCredentials

  ScaleIAMInstanceProfilesAccountsTest( ) {
  ...
  void test( ) {
    ...
    try {
      ...
      final int instanceProfiles = 100
      final int iterations = accounts / threads
      print( "Creating ${instanceProfiles} instance profiles in ${accounts} accounts using ${threads} threads" )
          ...
          try{
            ( 1..iterations ).each { Integer account ->
              ...
              getYouAreClient( accountCredentials ).with {
                print( "[${thread}] Creating ${instanceProfiles} instance profiles for account ${accountName}" )
                (1..instanceProfiles).each { Integer instanceProfile ->
                  ...
                  // let recursive account deletion clean up instance profiles
                  //cleanupTasks.add {
